# Usage Guide
This guide explores usage and configuration scenarios for Nova. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](../res/usage/first-task.png)

## Basic Operations
Nova provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Nova container without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Nova provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the Nova environment
...
### File Attachments
Nova supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Nova's power comes from its ability to use [tools](../developer/architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Nova includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.
...
Nova instances can communicate with each other using the A2A protocol. This enables:
...
To enable A2A connectivity, go to **Settings → MCP/A2A → A0 A2A Server** and toggle the server on. You'll receive a connection URL that other Nova instances can use to communicate with your agent.

See [A2A Setup](a2a-setup.md) for detailed configuration and use cases.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Nova to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Nova Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Nova to solve complex problems.

## Multi-Agent Cooperation
One of Nova's unique features is multi-agent cooperation.
...
* **Hierarchy:** Nova uses a [hierarchical structure](../developer/architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](../res/usage/multi-agent.png)

## Projects
Projects are isolated workspaces that provide dedicated context, instructions, memory, and secrets for specific tasks or clients. They are one of Nova's most powerful organizational features, preventing context bleed and enabling focused, specialized agent behavior.
...
3. Nova clones the repository and sets up the project structure
...
> [!NOTE]
> If the cloned repository already contains a `.a0proj/` configuration folder, Nova merges the existing configuration with your specified preferences.
...
## Tasks & Scheduling
Tasks enable Nova to run automated or scheduled work in isolated contexts. They're perfect for recurring workflows, batch processing, or time-delayed operations that don't require immediate attention.

### What Are Tasks?

Tasks are autonomous work units that Nova executes in dedicated or shared chat contexts. Each task includes:
...
Nova supports three types of tasks:
...
Nova includes a secure tunneling feature that allows you to expose your local instance to the internet. This makes it possible to access your Nova instance from anywhere or share it with others without complex network configuration.

### How Tunneling Works

Nova uses the [Flaredantic](https://pypi.org/project/flaredantic/) library to create secure tunnels. These tunnels:
...
6. The tunnel URL remains active until you stop the tunnel or close Nova

### Security Considerations

When sharing your Nova instance via a tunnel:

- Anyone with the URL can access your Nova instance
- No additional authentication is added beyond what your Nova instance already has
- **Always set up authentication before creating a tunnel** (see below)
- The tunnel exposes only your Nova instance, not your entire system

> [!IMPORTANT]
> When attempting to create a tunnel without authentication configured, Nova will display a security warning.

### Adding Authentication for Tunnels

To secure your tunneled Nova instance, configure authentication in Settings:

1. Open **Settings** in the Nova UI
...
This will require users to enter these credentials when accessing your tunneled Nova instance.
...
> [!TIP]
> Combine tunneling with authentication for secure remote access to your Nova instance from any device, including mobile phones and tablets.

## Voice Interface
Nova provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Nova provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The file browser lets you navigate the Nova filesystem. For file-based work, keep your working files in `/a0/usr` (or inside a Project workspace).
>
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with Nova's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Memory Management
Nova includes a sophisticated memory management system that stores and retrieves information from conversations, knowledge sources, and learning experiences. The Memory Dashboard provides a powerful interface to view, search, filter, edit, and delete memory entries stored in the vector database.
...
## Backup & Restore
Nova provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Nova setups between different systems.
...
#### What Gets Backed Up
By default, Nova backs up your most important data:
...
The restore process allows you to recover your Nova setup from a previous backup:
...
* **Before System Updates**: Backup before updating Nova or system components
...
* **External Storage**: Keep backup files in a safe location outside Nova
...
#### System Migration
Moving Nova to a new server or computer:
1. Create a complete backup on the original system
2. Install Nova on the new system
...
> [!TIP]
> The backup system is designed to work across different operating systems and Nova installations. Your backups from a Windows system will work on Linux, and vice versa.
