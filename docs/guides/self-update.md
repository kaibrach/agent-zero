...
Nova includes a Docker-oriented self-update flow for switching to a specific repository version tag on `main`, `testing`, or `development`.
...
5. It fetches the requested branch and update target from the official Nova repository.
6. It updates `/a0` while preserving gitignored paths such as `/a0/usr`.
7. It starts Nova again and waits for `/api/health` to become healthy.
...
Nova version tags follow this format:
