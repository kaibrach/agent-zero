...
One of Nova's most powerful capabilities is the ability to integrate external APIs on the fly without writing any code. By simply providing API documentation or code snippets, the agent can learn to use new services and remember how to use them in future conversations.
...
### Step 2: Provide the Code to Nova

Now we'll tell Nova to use this code:

1. Open a chat with Nova
2. Send a message like: **"Use this code to generate an image of the Nova logo"**
3. Either:
   - Attach the downloaded Python file, or
   - Paste the code snippet into the chat

Nova will analyze the code and understand:
- What dependencies are needed (`google-genai` package)
- How to structure the API request
- What parameters are required
- How to handle the response

![Nova analyzing the API code](../res/usage/api-int/api-int-2image-gen-api2.png)

### Step 3: Configure API Credentials

On first run, Nova will attempt to use the API but discover it needs an API key:
...
After configuring the API key, tell Nova to proceed:
...
Here's where it gets interesting: **Nova will remember this solution**.
...
For APIs you use frequently, you can have Nova create reusable wrapper functions:
...
By leveraging Nova's ability to learn from API code snippets, you can:
...
This makes Nova incredibly versatile - it's not limited to its built-in features. Show it how to use an API once, and it becomes part of its permanent toolkit.
