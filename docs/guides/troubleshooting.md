...
This page addresses frequently asked questions (FAQ) and provides troubleshooting steps for common issues encountered while using Nova.

## Frequently Asked Questions
**1. How do I ask Nova to work directly on my files or dirs?**
- Place the files/dirs in `/a0/usr`. Nova will be able to perform tasks on them.
...
**6. How do I integrate open-source models with Nova?**
...
**7. How can I make Nova retain memory between sessions?**
Use **Settings → Backup & Restore** and avoid mapping the entire `/a0` directory. See [How to update Nova](../setup/installation.md#how-to-update-agent-zero).
...
- Join the Nova [Skool](https://www.skool.com/agent-zero) or [Discord](https://discord.gg/B8KZKNsPpj) community.
...
**13. Can Nova interact with external APIs or services (e.g., WhatsApp)?**
...
The recovery command only schedules the update. Restart the container or let Nova start again, then check `/exe/a0-self-update.log` and `/exe/a0-self-update-status.yaml` to see what happened.
...
* **Performance Issues:** If Nova is slow or unresponsive, it might be due to resource limitations, network latency, or the complexity of your prompts and tasks, especially when using local models.
