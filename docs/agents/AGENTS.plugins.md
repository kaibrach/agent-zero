...
This guide covers the Python Backend and Frontend WebUI plugin architecture. Use this as the definitive reference for building and extending Nova.

---

## 1. Architecture Overview

Nova uses a convention-over-configuration plugin model where runtime capabilities are discovered from the directory structure.
...
Plugins can include an optional `hooks.py` file at the plugin root. Nova loads this module on demand and calls exported functions by name through `helpers.plugins.call_plugin_hook(...)`.

- `hooks.py` runs inside the **Nova framework runtime and Python environment**, not the separate agent execution environment.
- Use it for framework-internal operations such as install-time setup, pre-update cleanup or preparation, plugin registration work, filesystem preparation, cache updates, or other tasks that need access to Nova internals.
...
- If `hooks.py` installs Python packages with `sys.executable -m pip`, those packages are installed into the **same Python environment that runs Nova itself**.
...
The **Plugin Index** is a community-maintained repository at https://github.com/agent0ai/a0-plugins that lists plugins available to the Nova community. Plugins listed there can be discovered and installed by other users.
...
**Runtime manifest** (`plugin.yaml`, inside your plugin repo/directory — drives Nova behavior):
```yaml
...
Both routes surface Plugin Index entries inside Nova. The Plugin Hub supports search, filtering, sorting, and a detail view with README content and installation actions.
