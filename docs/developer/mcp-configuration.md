...
This guide provides comprehensive technical details for configuring and utilizing external tool providers through the Model Context Protocol (MCP) with Nova. This allows Nova to leverage tools hosted by separate local or remote MCP-compliant servers.

> [!NOTE]
> For a quick start guide on adding MCP servers through the UI, see [MCP Setup](../guides/mcp-setup.md).

> [!NOTE]
> This guide covers Nova as an MCP **client**. To expose Nova as an MCP **server**, see [Connectivity → MCP Server](connectivity.md#mcp-server-connectivity).

## MCP Server Types

Nova supports three main types of MCP servers:

1.  **Local Stdio Servers**: Local executables that Nova communicates with via standard input/output (stdio).
...
## How Nova Consumes MCP Tools

Nova discovers and integrates MCP tools dynamically through the following process:

1.  **Configuration**: MCP servers are defined in the Nova configuration, primarily through the Settings UI.
2.  **Saving Settings**: When saved via the UI, Nova updates `usr/settings.json`, specifically the `"mcp_servers"` key.
3.  **Server Startup**: Nova initializes configured MCP servers (stdio) or connects to them (remote). For `npx`/`uvx` based servers, the first run downloads packages.
4.  **Tool Discovery**: Upon initialization, Nova connects to each enabled MCP server and queries for available tools, descriptions, and parameters.
...
6.  **Tool Invocation**: When the LLM requests an MCP tool, Nova's `process_tools` method (`mcp_handler.py`) routes the request to the appropriate MCP server.
...
### Nova in Docker, MCP Server on Host
...
- Final tool name in Nova: `sequential_thinking.run_chain`
...
- Nova maintains persistent connections to remote servers
...
Nova can chain these tools automatically based on task requirements.
...
- Test with Nova before production
...
- [Connectivity: MCP Server](connectivity.md#mcp-server-connectivity) - Exposing Nova as MCP server
