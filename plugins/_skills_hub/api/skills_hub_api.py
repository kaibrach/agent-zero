...
from helpers import runtime, skills, files
from helpers.skills_import import resolve_skills_destination_root
from plugins._skills_hub.helpers.agentskill_api import (
    search_registry,
    fetch_skill_payload,
...
class Skills_Hub(ApiHandler):
    async def process(self, input: Input, request: Request) -> Output:
        action = input.get("action", "")

        try:
            if action == "registry_search":
    ...
    def registry_search(self, input: Input):
        ...
        payload = search_registry(query=query, section=section, limit=limit, page=page)
    ...
    def _prepend_owner_registry_matches(self, query: str, results: list, limit: int) -> list:
        ...
        fallback_items = []
        for owner_slug in fetch_owner_skill_slugs(owner, limit=limit):
    ...
    def _fetch_registry_install_payload(self, owner: str, slug: str) -> dict:
        ...
        return fetch_skill_payload(registry_slug)
    ...
    def _fetch_quality_details(self, url: str) -> dict:
        lines = fetch_registry_page_lines(url)
        if not lines:
            return {"url": url}
